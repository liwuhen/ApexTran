...
def run(
    ...
    framework = ctx.ensure_object(ApexTranFramework)
...
def _default_project() -> Path:
    ...
    project = settings.home / "ApexTran-project"
...
project_opt = typer.Option(
    ...
    help="Path to the project directory (default: ~/.ApexTran/ApexTran-project)",
    envvar="ApexTran_PROJECT",
)
...
def _uv(*args: str, cwd: Path) -> subprocess.CompletedProcess:
    ...
    if not _is_in_venv():
        typer.secho("Please install ApexTran in a virtual environment to use this command.", err=True, fg="red")
        raise typer.Exit(1)
...
ApexTran_CONTRIB_REPO = "https://github.com/bubbuild/bub-contrib.git"
...
def _build_requirement(spec: str) -> str:
    if spec.startswith(("git@", "https://")):
        ...
        if has_ref:
            ref = f"@{ref}"
            return f"git+{ApexTran_CONTRIB_REPO}{ref}#subdirectory=packages/{name}"
...
def _build_ApexTran_requirement() -> list[str]:
    dist = metadata.distribution("ApexTran")
    dist_name = dist.name
...
def _ensure_project(project: Path) -> None:
    if (project / "pyproject.toml").is_file():
        return
    _uv("init", "--bare", "--name", "ApexTran-project", "--app", cwd=project)
    ApexTran_requirement = _build_ApexTran_requirement()
    _uv("add", "--active", "--no-sync", *ApexTran_requirement, cwd=project)
...
def install(
    ...
    """Install a plugin into ApexTran's environment, or sync the environment if no specifications are provided."""
...
def uninstall(
    ...
    """Uninstall a plugin from ApexTran's environment."""
    _ensure_project(project)
...
def update(
    ...
    """Update selected package or all packages in ApexTran's environment."""
    _ensure_project(project)
    if not packages:
