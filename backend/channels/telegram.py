...
NO_ACCESS_MESSAGE = "You are not allowed to chat with me. Please deploy your own instance of ApexTran."
...
class ApexTranMessageFilter(filters.MessageFilter):
    ...
    def _mentions_bot(self, message: Message, bot_id: int, bot_username: str) -> bool:
        ...
        mentions_by_keyword = "ApexTran" in content or bool(bot_username and f"@{bot_username}" in content)
...
MESSAGE_FILTER = ApexTranMessageFilter()
...
class TelegramChannel(Channel):
    ...
    async def start(self, stop_event: asyncio.Event) -> None:
        ...
        self._app.add_handler(CommandHandler("ApexTran", self._on_message, has_args=True, block=False))
    ...
    async def _on_start(self, update: Update, _context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if self._allow_chats and str(update.message.chat_id) not in self._allow_chats:
            ...
            return
        await update.message.reply_text("ApexTran is online. Send text to start.")
    ...
    async def _build_message(self, message: Message) -> ChannelMessage:
        ...
        if content.startswith("/ApexTran "):
            content = content[len("/ApexTran ") :]
