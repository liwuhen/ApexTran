...
def main():
    ...
    parser.add_argument("--token", "-t", help="Bot token (defaults to ApexTran_TELEGRAM_TOKEN env var)")
    ...
    # Get bot token
    bot_token = args.token or os.environ.get("ApexTran_TELEGRAM_TOKEN")
    if not bot_token:
        print("❌ Error: Bot token required. Set ApexTran_TELEGRAM_TOKEN env var or use --token")
        sys.exit(1)
