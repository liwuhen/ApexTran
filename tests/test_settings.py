...
def test_settings_single_api_key_and_base() -> None:
    settings = _settings_with_env({"ApexTran_API_KEY": "sk-test", "ApexTran_API_BASE": "https://api.example.com"})
...
def test_settings_per_provider_keys() -> None:
    settings = _settings_with_env({
        "ApexTran_OPENAI_API_KEY": "sk-openai",
        "ApexTran_OPENAI_API_BASE": "https://api.openai.com",
        "ApexTran_ANTHROPIC_API_KEY": "sk-anthropic",
    })
...
def test_settings_provider_names_are_lowercased() -> None:
    settings = _settings_with_env({"ApexTran_OPENROUTER_API_KEY": "sk-or"})
...
""".strip(),
    ...
    with patch.dict("os.environ", {"ApexTran_HOME": str(tmp_path)}, clear=True):
...
""".strip(),
    ...
    with patch.dict(
        "os.environ",
        {
            "ApexTran_HOME": str(tmp_path),
            "ApexTran_MODEL": "anthropic:claude-3-7-sonnet",
            "ApexTran_API_KEY": "sk-env",
            "ApexTran_CLIENT_ARGS": '{"extra_headers":{"HTTP-Referer":"https://env.example","X-Title":"Env App"}}',
            "ApexTran_MAX_STEPS": "12",
        },
        clear=True,
    ):
        settings = AgentSettings()
...
def test_settings_client_args_can_be_disabled() -> None:
    settings = _settings_with_env({"ApexTran_CLIENT_ARGS": "null"})
