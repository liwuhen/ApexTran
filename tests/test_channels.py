...
from backend.channels.telegram import ApexTranMessageFilter, TelegramChannel, TelegramMessageParser
...
def test_bub_message_filter_accepts_private_messages() -> None:
    ...
    assert ApexTranMessageFilter().filter(message) is True
...
def test_bub_message_filter_requires_group_mention_or_reply() -> None:
    ...
    assert ApexTranMessageFilter().filter(message) is False
...
async def test_feishu_handle_event_payload_builds_channel_message(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ApexTran_FEISHU_APP_ID", "cli_dummy_app_id")
    monkeypatch.setenv("ApexTran_FEISHU_APP_SECRET", "dummy-feishu-app-secret")
