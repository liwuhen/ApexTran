...
from backend.hooks.hookspecs import ApexTran_HOOK_NAMESPACE, ApexTranHookSpecs, hookimpl


def _runtime_with_plugins(*plugins: tuple[str, object]) -> HookRuntime:
    manager = pluggy.PluginManager(ApexTran_HOOK_NAMESPACE)
    manager.add_hookspecs(ApexTranHookSpecs)
